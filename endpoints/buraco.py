...
router = APIRouter(prefix="/buraco",
                   tags=["Buraco"],
...
@router.post("/", response_description="Buraco added into the database")
...
@router.put("/{buraco_id}")
async def update_buraco(buraco_id: str, buraco_update_req: BuracoUpdateRequest):
    session = database.get_db_session(engine)

    try:
        is_buraco_updated = session.query(Buraco).filter(Buraco.id == buraco_id).update(
            {Buraco.idTamanhoBuraco: buraco_update_req.tamanho_buraco_id, Buraco.votos: buraco_update_req.votos}, synchronize_session=False)
        ...
        if is_buraco_updated == 1:
            data = session.query(Buraco).filter(
                Buraco.id == buraco_id).one()
        elif is_buraco_updated == 0:
            response_msg = "Buraco não atualizado. Nenhum Buraco encontrado com o id: " + \
                str(buraco_id)
            error = True
            data = None
...
@router.delete("/{buraco_id}")
async def delete_buraco(buraco_id: str):
    session = database.get_db_session(engine)

    try:
        is_buraco_updated = session.query(Buraco).filter(and_(Buraco.id == buraco_id, Buraco.apagado == False)).update({
            Buraco.apagado: True
        ...
        if is_buraco_updated == 0:
            response_msg = "Buraco não apagado. Nenhum Buraco encontrado com o id: " + \
                str(buraco_id)
            error = True
            data = None
        return Response(data, response_code, response_msg, error)
...
@router.get("/{buraco_id}")
async def read_buraco(buraco_id: str):
    session = database.get_db_session(engine)
    response_msg = "Buraco encontrado com sucesso"
    data = None
    try:
        data = session.query(Buraco).filter(and_(
            Buraco.id == buraco_id, Buraco.apagado == False)).one()
    except Exception as ex:
        print("Error", ex)
